...
tuplaConstructor = tuple(
    ("Esta", "es", "una", "tupla")
)  # con el constructur de tupla podemos iniciar una
...
vehiculos = tuple(
    listaVehiculos
)  # asignar a la tupla la lista modificada convertida en tupla

# Desempaquetamiento:
(a, b, c, d, e, f, g) = (
    vehiculos  # se asignará cada elemento a una variable correspondiente a la posicion
)
(bici, moto, *cuadroRuedas, avion, barco, crucero) = (
...
for i in range(
    len(vehiculos)
):  # De esta forma podemos tener el índice del elemento iterable al recorrer la tupla
    print(f"{i+1}. {vehiculos[i]}")

[
    print(tecnologia) for tecnologia in vehiculos
]  # Este sería el shorthand para una impresión de tupla iterable
