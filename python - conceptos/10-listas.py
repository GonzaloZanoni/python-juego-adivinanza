...
listaDesdeConstructor = list(
    ("Beta", 2, True)
)  # Hay que utilizar el constructor list y doble paréntesis
...
if (
    "Manzana" in frutas
):  # Utilizando la palabra "in" podemos saber si un elemento está presente
...
tecnologias.remove(
    "Vue"
)  # Se remueve el elemneto que coincida con lo que pasemos como argumetno
...
for i in range(
    len(tecnologias)
):  # De esta forma podemos tener el índice del elemento iterable al recorrer la lista
    print(f"{i+1}. {tecnologias[i]}")

[
    print(tecnologia) for tecnologia in tecnologias
]  # Este sería el shorthand para una impresión de lista iterable
...
lista2conY = [
    tecnologia.upper() for tecnologia in tecnologias if "y" in tecnologia
]  # Shorthand del ejemplo de arriba
