...
longitud = len(
    conjunto
)  # la longitud o cantidad de elmentos del conjunto (no tomará en cuenta los duplicados)
tipo = type(conjunto)  # para saber el tipo <class 'set'>
conjuntoConstructor = set(
    ("Este", "es", "un", "conjunto")
)  # creamos un conjunto usando el contructor
...
telefonos.update(
    telefonos2
)  # Con update sumamos otra colección(puede ser cualquier colección) a nuestro conjunto
...
autos.remove(
    "Ferrari"
)  # Se borra un elemento que coincida exactamente con lo pasado por argumento (Da un error si no existe)
autos.discard(
    "Fiat"
)  # Se borra un elemento que coincida exactamente con lo pasado por argumento (no da un error si no existe)
...
booleanos_union = a.union(
    booleanos
)  # True y 1 se consideran el mismo valor por lo cual solo se agregará False
booleanos_intersection = a.intersection(
    booleanos
)  # La única intersección es True ya que se considera igual a 1
