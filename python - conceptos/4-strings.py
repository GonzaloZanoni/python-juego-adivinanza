...
print(
    texto[11:16]
)  # Se imprimirá el fragmento que vaya desde el índice 11 al 16 (no incluído)
print(
    texto[:5]
)  # Se imprimirá el fragmento que vaya desde el comienzo al índice 5 (no incluído)
print(
    texto[38:]
)  # Se imprimirá el fragmento que vaya desde el índice 38 hasta el final
print(
    texto[-4:]
)  # Se imprimirá el fragmento que vaya desde 4 índices antes del final hasta el final
print(
    texto[:-37]
)  # Se imprimirá el fragmento desde el comienzo hasta el final 37 caracteres antes del final
...
sin_espacios = (
    texto_con_espacios.strip()
)  # Eliminará los espacios del comienzo y el final
reemplazo = texto_con_espacios.replace(
    "mundo", "universo"
)  # Reeamplazará mundo por universo
separar_por_comas = texto_con_comas.split(
    "/"
)  # Separará el texto por comas y nos devolverá una lista

# Métodos más usados
texto_a_modificar = "este es UN texto A MODIFICAR"
capitalizado = (
    texto_a_modificar.capitalize()
)  # Sólo quedará la primera letra como mayúscula
...
encontrador = texto_a_modificar.find(
    "texto"
)  # Nos devuelve el índice donde comienza la palabra buscada
...
directorio = (
    "El directorio donde está almacenado mi mayor secreo es c:\\secreto\\archivo.txt"
)
